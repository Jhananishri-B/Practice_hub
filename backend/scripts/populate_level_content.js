...
async function run() {
    ...
    try {
        ...
        for (const [courseTitle, levelsData] of Object.entries(CONTENT_MAP)) {
            const [courses] = await connection.query('SELECT id FROM courses WHERE title LIKE ?', [`%${courseTitle}%`]);
            if (courses.length === 0) {
            ...
            for (const [levelNum, data] of Object.entries(levelsData)) {
                console.log(`Updating ${courseTitle} Level ${levelNum}...`);
                await connection.query(
                    `UPDATE levels 
           SET title = ?, learning_materials = ?
           WHERE course_id = ? AND level_number = ?`,
                    [data.title, JSON.stringify(data.content), courseId, levelNum]
                );
            }
        }
