...
async function run() {
    ...
    try {
        ...
        for (const [courseTitle, levelsData] of Object.entries(CONTENT_MAP)) {
            ...
            // Assuming the most relevant course is first (or distinct).
            // We removed duplicates earlier, so should be safe.
            const courseId = courses[0].id;

            for (const [levelNum, content] of Object.entries(levelsData)) {
                console.log(`Updating ${courseTitle} Level ${levelNum}...`);

                await connection.query(
                    `UPDATE levels 
           SET learning_materials = ?
           WHERE course_id = ? AND level_number = ?`,
                    [JSON.stringify(content), courseId, levelNum]
                );
            }
        }
