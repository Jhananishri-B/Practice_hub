    ...
    return (
        <Layout>
            <div className="flex-1 p-6 overflow-y-auto">
                ...
                {/* Score Summary Card */}
                <div className="mb-6 p-6 bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700 flex items-center justify-between">
                    <div>
                        <p className="text-gray-500 dark:text-gray-400 font-medium uppercase tracking-wide text-xs">Total Score</p>
                        <div className="flex items-baseline gap-2">
                            <span className={`text-4xl font-bold ${scorePercentage >= 70 ? 'text-green-600 dark:text-green-400' : 'text-blue-600 dark:text-blue-400'}`}>
                                {scorePercentage}%
                            </span>
                            <span className="text-gray-400">({correctCount}/{totalCount} correct)</span>
                        </div>
                    </div>
                    <div className="text-right">
                        <div className="text-sm text-gray-500 dark:text-gray-400">
                            {scorePercentage >= 80 ? 'Excellent work!' : scorePercentage >= 60 ? 'Good effort!' : 'Keep practicing!'}
                        </div>
                    </div>
                </div>
                ...
                <div className="mb-6 flex gap-2 flex-wrap">
                    {results.questions.map((q, index) => (
                        <button
                            ...
                            className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-colors ${index === selectedQuestionIndex
